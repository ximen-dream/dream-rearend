...
public class ValidateCodeFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        ...
        String client_id = getClientId(header, httpServletRequest);
        if (matcher.matches(httpServletRequest)
                && StringUtils.equalsIgnoreCase(grant_type, "password")
                && !StringUtils.equalsAnyIgnoreCase(client_id, "swagger")) {
            try {
                validateCode(httpServletRequest);
                filterChain.doFilter(httpServletRequest, httpServletResponse);
            } catch (ValidateCodeException e) {
                DreamResponse febsResponse = new DreamResponse();
                DreamUtil.makeResponse(httpServletResponse, MediaType.APPLICATION_JSON_UTF8_VALUE,
